    ...
    <div class="backgroundcolor-position">
        ...
        <div class="main-content">
                ...
                <form action="{{ route('catpost.update', $post->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="container-flex">
                        <div class="flexblock">
                            ...
                            <br>
                            <input type="number" class="textbox-age" min="0" max="30" id="age"
                                name="age" placeholder="例：2（才）" value="{{ old('age', $post->age) }}" />
                        ...
                        <div class="flexblock">
                            ...
                            <select name="gender" id="gender" class="textbox-gender">
                                ...
                                @foreach ([0 => '未入力', 1 => 'オス', 2 => 'メス'] as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ $key == old('gender', $post->gender) ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                    ...
                    <div class="container">
                        <div class="flexblock">
                            <label>品種</label><br>
                            <input type="text" class="textbox-kinds" id="breed" name="breed" placeholder="例：ミックス"
                                value="{{ old('breed', $post->breed) }}" />
                        </div>
                        ...
                        <div class="flexblock">
                            <label>所在地</label><br>
                            <input type="text" class="textbox-location" id="region" name="region"
                                placeholder="都道府県を入力（例：東京都）" value="{{ old('region', $post->region) }}" />
                        </div>
                    ...
                    <select name="status" id="status" class="textbox-status">
                        @foreach ([0 => '里親募集中', 1 => 'お見合い中', 2 => '譲渡成立'] as $key => $label)
                            <option value="{{ $key }}"
                                {{ $key == old('status', $post->status) ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <div class="container-flex date-range">
                        <div class="bbb">
                            ...
                            <input type="date"
                                ...
                                name="start_date" class="textbox-start-date"
                        ...
                        <div class="ccc">
                            ...
                            <input type="date"
                                ...
                                name="end_date" class="textbox-end-date"
                ...
                <div id="media-container" class="media-preview-grid">
                    ...
                    @foreach ($post->images as $image)
                        <div class="preview-item">
                            ...
                            <button type="button" class="remove-btn" data-type="image"
                    ...
                    @foreach ($post->videos as $video)
                        <div class="preview-item" style="width: 150px; height: 150px;">
                            ...
                            <button type="button" class="remove-btn" data-type="video"
                                data-id="{{ $video->id }}">×</button>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="background-health">
                ...
                <label>予防接種</label><br>
                <textarea class="textbox-vaccine" rows="3" cols="30" id="vaccination" name="vaccination"
                    placeholder="予防接種関連について詳しく記述してください。">{{ old('vaccination', $post->vaccination) }}</textarea>
                ...
                <label>病歴</label><br>
                <textarea class="textbox-disease" rows="3" cols="30" id="medical_history" name="medical_history"
                    placeholder="病歴等ございましたら詳しく記述してください。">{{ old('medical_history', $post->medical_history) }}</textarea>
            ...
            <div class="background-description">
                ...
                <label>詳細説明</label><br>
                <textarea rows="4" cols="30" id="description" name="description" class="textbox-description"
                    placeholder="猫の性格や特徴などを詳しく書いてください。">{{ old('description', $post->description) }}</textarea>
            ...
            <div class="background-price">
                ...
                <input type="text" data-type="number" class="textbox-price" id="cost" name="cost"
                    placeholder="例：30,000（円）" value="{{ old('cost', $post->cost) }}" />
