                ...
                <div class="dm-user-icon">
                    ...
                    @if (!empty($partner->image_path))
                        ...
                        <img src="{{ Storage::disk(config('filesystems.default'))->url('profile_images/' . $partner->image_path) }}"
                            alt="{{ $partner->name }}" class="user-image">
                    @else
        ...
        <div id="dm-messages" class="dm-messages">
            @foreach ($messages as $message)
                <div class="dm-message {{ $message->user_id === auth()->id() ? 'mine' : 'other' }}"
                    ...
                    @else
                        <div class="dm-actions">
                            <form action="{{ route('report.message', ['dm' => $dmId->id, 'message' => $message->id]) }}"
                                method="POST" onsubmit="return confirm('このメッセージを通報しますか？');">
                                @csrf
...
@section('script')
    <div id="dm-config"
        data-fetch-url="{{ route('dm.message.fetch', $dmId->id) }}"
        data-send-url="{{ route('dm.message.send', $dmId->id) }}"
