...
<li class="nav-item dropdown user-menu">
    ...
    <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
        ...
        <li class="user-footer">
            <form id="delete-account-form" action="{{ route('admin.user.delete') }}" {{-- ← 退会ルート名に合わせて変更 --}}
                method="POST" onsubmit="return confirm('本当に退会しますか？\nこの操作は取り消せません。');">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger btn-block">
                    <i class="fa fa-fw fa-user-times text-white"></i> アカウントを削除（退会）
                </button>
            </form>
        </li>
