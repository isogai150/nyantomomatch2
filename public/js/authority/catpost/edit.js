...
$(function() {
     ...
     * 残り画像枚数を計算
     */
    function calculateRemainingImageCount() {
    ...
    function updateButtonStates() {
        const remaining = calculateRemainingImageCount();
     ...
     * 新規画像削除（送信前）
     */
    previewContainer.on('click', '.remove-btn.new', function(event) {
     ...
     * 新規動画削除
     */
    $(document).on('click', '.remove-btn.video-new', function(event) {
        event.preventDefault();
        event.stopPropagation();
     ...
     * 既存メディア削除（Ajax）
     */
    $(document).on('click', '.remove-btn[data-type]', function(event) {
        event.preventDefault();
        event.stopPropagation();
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        // その他のエラー
                        const errorDiv = $('<div class="alert-danger" style="text-align: center; margin: 20px 0;"></div>')
